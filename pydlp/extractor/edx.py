...
class EdXIE(InfoExtractor):
    IE_NAME = "edx"
    IE_DESC = "edX course video and lecture extractor"
    _VALID_URL = r"https?://(?:www\.)?edx\.org/course/(?:[^/]+/)*(?P<id>[a-zA-Z0-9_-]+)"

    def _real_extract(self, url: str) -> MediaInfo:
        video_id = self._match_id(url)
        webpage = self._download_webpage(url, video_id=video_id)

        title = self._html_search_meta(["og:title", "twitter:title"], webpage, default=f"edX Course {video_id}")
