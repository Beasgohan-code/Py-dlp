...
class VeohIE(InfoExtractor):
    IE_NAME = "veoh"
    IE_DESC = "Veoh.com video extractor"
    _VALID_URL = r"https?://(?:www\.)?veoh\.com/watch/(?P<id>[a-zA-Z0-9]+)"

    def _real_extract(self, url: str) -> MediaInfo:
        video_id = self._match_id(url)
        webpage = self._download_webpage(url, video_id=video_id)

        title = self._html_search_meta(["og:title", "twitter:title"], webpage, default=f"Veoh Video {video_id}")
        ...
        for m in re.finditer(r'["\'](https?://[^"\']+\.(?:mp4|m3u8)[^"\']*)["\']', webpage):
            src = m.group(1)
            if ".m3u8" in src and "veoh" in src:
                formats.extend(self._extract_m3u8_formats(src, video_id=video_id, fatal=False))
            elif ".mp4" in src and "veoh" in src and not any(f.url == src for f in formats):
